...
might be a useful reminder later.
...
The commandline or `terminal' is a text-based interface you can use to
run programs and analyse your data. If this is your first time using one
it will seem pretty daunting at first but, with just a few commands,
you'll start to see how it helps you to get things done more
efficiently.

    \hypertarget{getting-started}{%
\subsection{Getting started}\label{getting-started}}

When working on the commandliine you will always work in a specific
location or directory in the filesystem. So let's check that you're in
the right place. Type the command below in the terminal window followed
by the \texttt{Enter} key:
...
\end{tcolorbox}
    ...
    The \texttt{pwd} command stands for print working directory. It should
display something similar to:

    \texttt{/home/manager/course\_data/linux/data}

    This means you are working in the \texttt{data} directory found inside
the \texttt{linux} directory inside the \texttt{course\_data} directory
and so on. There will be more on the linux filesystem structure shortly.

Now continue through this tutorial, entering any commands that you
encounter (highlighted in a grey box with a keyboard symbol) into your
terminal window followed by the \texttt{Enter} key. Let's start by
...
\begin{center}
\includegraphics[Output of the list command]{images/ls_output.png}
\end{center}


    \hypertarget{commands-options-and-arguments}{%
\subsection{Commands, options and
arguments}\label{commands-options-and-arguments}}

A command consists of three parts: the \textit{command}, \textit{options},
and \textit{arguments}. Of these, the command is required while options
and arguments are optional. Options are used to modify the default
behavior of the command. Arguments are used to provide input to the
command.

\texttt{command\ -options\ arguments}

In the previous call to \texttt{ls}, the command is \texttt{ls} and
there are no options or arguments passed to the \texttt{ls} command.

Now let's add some options, here the command is \texttt{ls}, and
\texttt{-a} and \texttt{-l} are options:
...
\end{tcolorbox}
    ...
    \textbf{Note} that there is a space between the command \texttt{ls} and
the options \texttt{-a} and \texttt{-l}. There is no space between the
dashes and the letters \texttt{a} and \texttt{l}.

The \texttt{-a} option will list \textit{all} contents of a directory
including hidden files and directories (hidden files and directories are
not shown by default and are used to help important data from being
deleted) and the \texttt{-l} option will print additional information
about each item in the directory (size, owner, date changed etc.).

You can also combine the \texttt{-a} and \texttt{-l} options into what
appears to be a single \texttt{-al} option. It's almost always ok to do
...
\end{tcolorbox}
    ...
    Now let's add an argument, here the command is \texttt{ls}, the options
are \texttt{-a} and \texttt{-l} and \texttt{basic/Pfalciparum} is the
argument:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }\PYZhy{}al\PY{+w}{ }basic
\end{Verbatim}
\end{tcolorbox}

    The argument instructs the \texttt{ls} command to display the content of
the specified directory, in this case the \texttt{basic} directory
instead of the current directory.

    There are some general points to remember that will make your life
easier:
...
\item
  ...
  \texttt{LS}.
\item
  \texttt{ls} is the letter \texttt{l} followed by the letter \texttt{s}
  (not the number one).
\item
  Often when you have problems with Linux, it is due to a spelling
  mistake. Check that you have not misspelled a command or missed a
  space between the command and the options and arguments. Pay careful
  attention if typing commands across multiple lines.
\end{itemize}
...
different directories to keep them separate.

The start of the filesystem is known as the root directory and denoted
with \texttt{/} symbol. The location or directory that you are in at any
given time is referred to as the current working directory denoted with
the \texttt{.} symbol.

Every file and directory on the computer is found in a specific
location. The location is specified as a path to that file or directory
and can be expresed as either the \textit{relative path} or \textit{absolute
path}.

The \textit{relative path} is the location of a file or directory from the
current working directory (.).

The \textit{absolute path} is the location of a file or directory from the
start of the file system or root directory (/).

    For example, for the file called \texttt{directory\_structure.png} under
the \texttt{linux} directory, the location or \textit{relative path} can
be expressed as:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }./basic/directory\PYZus{}structure.png
\end{Verbatim}
\end{tcolorbox}

    The location of this file can also be expressed using the \textit{absolute
path} as:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }/home/manager/course\PYZus{}data/linux/data/basic/directory\PYZus{}structure.png
\end{Verbatim}
\end{tcolorbox}
...
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
tree\PY{+w}{ }/home/manager/course\PYZus{}data/linux
\end{Verbatim}
\end{tcolorbox}
...
Linux commands! A useful command to obtain further information on any
...
\end{tcolorbox}
    ...
    To exit out of the man command type \texttt{q}.

    \hypertarget{find---find-a-file}{%
\subsection{find - find a file}\label{find---find-a-file}}

The \texttt{find} command can be used to find files in a directory
hierarchy that match a specific criteria. For example, it can be used to
recursively search the directory tree for a specific file name or
pattern, seeking files and directories that match the given name or
pattern.

    To find all files in the current directory (.) named
\texttt{Pfalciparum.fa}:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
find\PY{+w}{ }.\PY{+w}{ }\PYZhy{}name\PY{+w}{ }\PY{l+s+s2}{\PYZdq{}Pfalciparum.fa\PYZdq{}}
\end{Verbatim}
\end{tcolorbox}

    To find all files in the current directory (.) and all its
subdirectories that have the suffix \texttt{.bed}:

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
find\PY{+w}{ }.\PY{+w}{ }\PYZhy{}name\PY{+w}{ }\PY{l+s+s2}{\PYZdq{}*.bed\PYZdq{}}
\end{Verbatim}
\end{tcolorbox}

    How many bed files did you find?

    Can you construct a command to find all the subdirectories contained in
the current directory? \textbf{Hint:} You may need to go back to reading
the manual page for the \texttt{find} command and search for the
\texttt{-type} option. Don't spend too much time on this as the solution
will be provided and explained later.
...
\end{tcolorbox}
    ...
    Now instead of typing out \texttt{ls\ Styphi/}, try typing
\texttt{ls\ S} and then press the \texttt{tab} key (instead of
\texttt{Enter}). The rest of the folder name should just appear.

If you have two files or directories with simiar names
(e.g.~\texttt{directory\_structure.png} and
\texttt{directory\_structure2.png}) then you might need to give your
terminal a bit of a hand to work out which one you want. Type the
following and then press the \texttt{tab} key (instead of
\texttt{Enter}).

    \begin{tcolorbox}[breakable, size=fbox, boxrule=1pt, pad at break*=1mm,colback=cellbackground, colframe=cellborder]
\prompt{In}{incolor}{ }{\boxspacing}
\begin{Verbatim}[commandchars=\\\{\}]
ls\PY{+w}{ }\PYZhy{}l\PY{+w}{ }d
\end{Verbatim}
\end{tcolorbox}

    In this case, when you press \texttt{tab} the terminal reads
\texttt{ls\ -l\ directory\_structure}, you then need to type \texttt{2}
followed by another \texttt{tab} and it works out that you meant
\texttt{directory\_structure2.png}.
...
information about gene features in a genome. Take a look at the contents
of the first example BED file used in this section:
...
\end{tcolorbox}
    ...
    A bed file is a column-based file, each column contains information
about the feature and is seperated by a tab character. There can be more
than 10 columns, but only the first three are required to be a valid
file. The file format is described in full here:
...
Now we want to find all genes from chromosome 2 that are on the positive
strand. We can use the \textbar{} symbol which is also known as the pipe
symbol to extract the genes that are on the positive strand, using grep
a second time:
...
\end{tcolorbox}
    ...
    This \textit{pipes} (sends) the output of
\texttt{grep\ chr2\ gene\_expression.bed} into the input of
\texttt{grep\ +}.

Since \texttt{grep} is reporting a match to a string \textit{anywhere} on
a line, such simple searches can have undesired consequences. For
example, consider the result of doing a similar search for all the genes
in chromosome 1:
